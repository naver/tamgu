...
#ifndef Tamgubinmapi_h
#define Tamgubinmapi_h
...
class Tamgubinmapi;
//This typedef defines a type "binmapiMethod", which expose the typical parameters of a new Tamgu method implementation
typedef Tamgu* (Tamgubinmapi::*binmapiMethod)(Tamgu* contextualpattern, short idthread, TamguCall* callfunc);
//---------------------------------------------------------------------------------------------------------------------
class Tamgubinmapi : public TamguLockContainer {
    public:
    //We export the methods that will be exposed for our new object
    //this is a static object, which is common to everyone
    //We associate the method pointers with their names in the linkedmethods map
    static Exchanging basebin_hash<binmapiMethod> methods;
    ...
    basebin_hash<long> values;
    bool isconst;

    //---------------------------------------------------------------------------------------------------------------------
    Tamgubinmapi(TamguGlobal* g, Tamgu* parent = NULL) : TamguLockContainer(g, parent) {
    ...
    Tamgu* Atom(bool forced) {
        if (forced || !protect || reference) {
            locking();
            Tamgubinmapi * m = new Tamgubinmapi;
            m->values = values;
            unlocking();
            return m;
        }
    ...
    double Sum() {
        locking();
        double v = 0;
        basebin_hash<long>::iterator itx;
    ...
    Tamgu* Newvalue(Tamgu* a, short idthread) {
        ...
        if (a->isContainer()) {
            ...
            for (it->Begin(); it->End() == aFALSE; it->Next()) {
                m->values[(ushort)it->Keyshort()] = it->Valueinteger();
            }
            it->Release();
            return m;
        }
    ...
    static void AddMethod(TamguGlobal* global, string name, binmapiMethod func, unsigned long arity, string infos);
    ...
    Tamgu* MethodItems(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        basebin_hash<long>::iterator itr;
        Tamgubinmapi* item;
        Tamgu* vect = Selectavector(contextualpattern);
        for (itr = values.begin(); itr != values.end(); itr++) {
            item = new Tamgubinmapi;
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        ...
        return (this->*Tamgubinmapi::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
...
//--------------------------------------------------------------------------------------------------
class TamguIterationbinmapi : public TamguIteration {
    public:

    basebin_hash<long>::iterator it;
    Tamgubinmapi map;

    TamguIterationbinmapi(Tamgubinmapi* m, bool d, TamguGlobal* g = NULL) : TamguIteration(d, g) {
        map.values = m->values;
    }
