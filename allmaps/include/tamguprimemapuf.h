...
#ifndef Tamguprimemapuf_h
#define Tamguprimemapuf_h
...
class Tamguprimemapuf;
//This typedef defines a type "primemapufMethod", which expose the typical parameters of a new Tamgu method implementation
typedef Tamgu* (Tamguprimemapuf::*primemapufMethod)(Tamgu* contextualpattern, short idthread, TamguCall* callfunc);
//---------------------------------------------------------------------------------------------------------------------
class Tamguprimemapuf : public TamguLockContainer {
    public:
    //We export the methods that will be exposed for our new object
    //this is a static object, which is common to everyone
    //We associate the method pointers with their names in the linkedmethods map
    static Exchanging basebin_hash<primemapufMethod> methods;
    ...
    //---------------------------------------------------------------------------------------------------------------------
    Tamguprimemapuf(TamguGlobal* g, Tamgu* parent = NULL) : TamguLockContainer(g, parent) {
    ...
    Tamgu* Atom(bool forced) {
        if (forced || !protect || reference) {
            locking();
            Tamguprimemapuf * m = new Tamguprimemapuf;
            m->values = values;
            unlocking();
            return m;
        }
    ...
    double Sum() {
        locking();
        double v = 0;
        prime_hash<wstring, double>::iterator itx;
    ...
    Tamgu* Newvalue(Tamgu* a, short idthread) {
        ...
        if (a->isContainer()) {
            ...
            for (it->Begin(); it->End() == aFALSE; it->Next()) {
                m->values[it->Keyustring()]=it->Valuefloat();
            }
            it->Release();
            return m;
        }
        ...
        double val=a->Float();
        prime_hash<wstring,double>::iterator ist;
    ...
    static void AddMethod(TamguGlobal* global, string name, primemapufMethod func, unsigned long arity, string infos);
    ...
    Tamgu* MethodItems(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        prime_hash<wstring, double>::iterator itr;
        Tamguprimemapuf* item;
        Tamgu* vect = Selectavector(contextualpattern);
        for (itr=values.begin(); itr!=values.end();itr++) {
            item=new Tamguprimemapuf;
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        ...
        return (this->*Tamguprimemapuf::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
...
//--------------------------------------------------------------------------------------------------
class TamguIterationprimemapuf : public TamguIteration {
    public:

    prime_hash<wstring, double>::iterator it;
    Tamguprimemapuf map;

    TamguIterationprimemapuf(Tamguprimemapuf* m, bool d, TamguGlobal* g = NULL) : TamguIteration(d, g) {
        map.values = m->values;
    }
