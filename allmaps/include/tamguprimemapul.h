...
#ifndef Tamguprimemapul_h
#define Tamguprimemapul_h
...
class Tamguprimemapul;
//This typedef defines a type "primemapulMethod", which expose the typical parameters of a new Tamgu method implementation
typedef Tamgu* (Tamguprimemapul::*primemapulMethod)(Tamgu* contextualpattern, short idthread, TamguCall* callfunc);
//---------------------------------------------------------------------------------------------------------------------
class Tamguprimemapul : public TamguLockContainer {
    public:
    //We export the methods that will be exposed for our new object
    //this is a static object, which is common to everyone
    //We associate the method pointers with their names in the linkedmethods map
    static Exchanging basebin_hash<primemapulMethod> methods;
    ...
    prime_hash<wstring, BLONG> values;
    bool isconst;

    //---------------------------------------------------------------------------------------------------------------------
    Tamguprimemapul(TamguGlobal* g, Tamgu* parent = NULL) : TamguLockContainer(g, parent) {
    ...
    Tamgu* Atom(bool forced) {
        if (forced || !protect || reference) {
            locking();
            Tamguprimemapul * m = new Tamguprimemapul;
            m->values = values;
            unlocking();
            return m;
        }
    ...
    double Sum() {
        locking();
        double v = 0;
        prime_hash<wstring, BLONG>::iterator itx;
    ...
    Tamgu* Newvalue(Tamgu* a, short idthread) {
        ...
        if (a->isContainer()) {
            ...
            for (it->Begin(); it->End() == aFALSE; it->Next()) {
                m->values[it->Keyustring()]=it->Valuelong();
            }
            it->Release();
            return m;
        }
        ...
        BLONG val=a->Long();
        prime_hash<wstring,BLONG>::iterator ist;
    ...
    static void AddMethod(TamguGlobal* global, string name, primemapulMethod func, unsigned long arity, string infos);
    ...
    Tamgu* MethodItems(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        prime_hash<wstring, BLONG>::iterator itr;
        Tamguprimemapul* item;
        Tamgu* vect = Selectavector(contextualpattern);
        for (itr=values.begin(); itr!=values.end();itr++) {
            item=new Tamguprimemapul;
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        ...
        return (this->*Tamguprimemapul::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
...
//--------------------------------------------------------------------------------------------------
class TamguIterationprimemapul : public TamguIteration {
    public:

    prime_hash<wstring, BLONG>::iterator it;
    Tamguprimemapul map;

    TamguIterationprimemapul(Tamguprimemapul* m, bool d, TamguGlobal* g = NULL) : TamguIteration(d, g) {
        map.values = m->values;
    }
