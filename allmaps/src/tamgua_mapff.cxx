...
Exporting hmap<string, string> Tamgua_mapff::infomethods;
Exporting bin_hash<unsigned long> Tamgua_mapff::exported;
...
bool Tamgua_mapff::InitialisationModule(TamguGlobal* global, string version) {
    ...
    global->RecordMethods(Tamgua_mapff::idtype, Tamgua_mapff::exported);
...
Exporting Tamgu*  Tamgua_mapff::Put(Tamgu* idx, Tamgu* ke, short idthread) {
    ...
    values.set(idx->Float(),  ke->Float());
    return aTRUE;
}

Exporting Tamgu* Tamgua_mapff::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    if (!idx->isIndex()) {
