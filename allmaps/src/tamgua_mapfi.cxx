...
Exporting hmap<string, string> Tamgua_mapfi::infomethods;
Exporting bin_hash<unsigned long> Tamgua_mapfi::exported;
...
bool Tamgua_mapfi::InitialisationModule(TamguGlobal* global, string version) {
    ...
    global->RecordMethods(Tamgua_mapfi::idtype, Tamgua_mapfi::exported);
...
Exporting Tamgu*  Tamgua_mapfi::Put(Tamgu* idx, Tamgu* ke, short idthread) {
    ...
    values.set(idx->Float(),  ke->Integer());
    return aTRUE;
}

Exporting Tamgu* Tamgua_mapfi::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    if (!idx->isIndex()) {
