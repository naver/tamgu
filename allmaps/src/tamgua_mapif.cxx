...
void Tamgua_mapif::Setidtype(TamguGlobal* global) {
  Locking lock(classlock);
  if (Tamgua_mapif::methods.isEmpty())
    Tamgua_mapif::InitialisationModule(global,"");
}
...
bool Tamgua_mapif::InitialisationModule(TamguGlobal* global, string version) {
    ...
    global->RecordCompatibilities(Tamgua_mapif::idtype);
...
Exporting Tamgu* Tamgua_mapif::Eval(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    if (!idx->isIndex()) {
...
Exporting Tamgu* Tamgua_mapif::divideinteger(Tamgu* b, bool itself) {
    Locking* _lock = _getlocktamgu(b);
