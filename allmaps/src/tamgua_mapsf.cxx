...
Exporting hmap<string, string> Tamgua_mapsf::infomethods;
Exporting bin_hash<unsigned long> Tamgua_mapsf::exported;
...
bool Tamgua_mapsf::InitialisationModule(TamguGlobal* global, string version) {
    ...
    global->RecordMethods(Tamgua_mapsf::idtype, Tamgua_mapsf::exported);
...
Exporting Tamgu*  Tamgua_mapsf::Put(Tamgu* idx, Tamgu* ke, short idthread) {
    ...
    values.set(idx->String(),  ke->Float());
    return aTRUE;
}

Exporting Tamgu* Tamgua_mapsf::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    if (!idx->isIndex()) {
