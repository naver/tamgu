...
Exporting bin_hash<unsigned long> Tamgua_mapss::exported;
...
Exporting Tamgu*  Tamgua_mapss::Put(Tamgu* idx, Tamgu* ke, short idthread) {
    ...
    vl = ke->String();
    values.set(idx->String(),  vl);
    return aTRUE;
}

Exporting Tamgu* Tamgua_mapss::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    if (!idx->isIndex()) {
