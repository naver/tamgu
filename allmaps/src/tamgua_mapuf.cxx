...
Exporting hmap<string, string> Tamgua_mapuf::infomethods;
Exporting bin_hash<unsigned long> Tamgua_mapuf::exported;
...
bool Tamgua_mapuf::InitialisationModule(TamguGlobal* global, string version) {
    ...
    global->RecordMethods(Tamgua_mapuf::idtype, Tamgua_mapuf::exported);
...
Exporting Tamgu*  Tamgua_mapuf::Put(Tamgu* idx, Tamgu* ke, short idthread) {
    ...
    values.set(idx->UString(),  ke->Float());
    return aTRUE;
}

Exporting Tamgu* Tamgua_mapuf::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    if (!idx->isIndex()) {
