...
public:
    ...
    void replacecurrent(TAMGUCHAR c) {
        w.replacecurrent(c);
    }
...
public:
    ...
    bool factorize(TamguDoubleSideAutomaton& a, long first=1);
    bool addmap(hmap<string, string>& lexicon, TamguDoubleSideAutomaton& a);
    bool load(string name, TamguDoubleSideAutomaton& a);
    void loadarcs(ifstream& dump, hmap<uint32_t, uint32_t>&, TamguDoubleSideAutomaton& a);
    bool compile(string name, TamguDoubleSideAutomaton& a);
...
public:
    ...
    TamguDoubleSideAutomaton() : start(*this), ialphabet(false) {
        normalize = false;
        finalize = false;
        encoding_table = 1;
        ialphabet[0] = "";
    ...
    bool up(wstring& w, vector<string>& res, long threshold, short flags, short idthread);
    ...
    bool process(charRead& w, vector<string>& res, bool option, long threshold, short flags, short idthread);
    
    void merge(TamguDoubleSideAutomaton* aut);
...
public:
    ...
    void set(wstring& ww, TamguDoubleSideAutomaton* aa, vector<string>& rr) {
        a = aa;
        w = ww;
        res = &rr;
        sz = w.size();
    ...
    void set(TamguDoubleSideAutomaton* aa, vector<string>& rr, bool addf, long th) {
        threshold = th;
        a = aa;
        res = &rr;
        sz = w.size();
