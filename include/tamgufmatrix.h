...
#ifndef tamgufmatrix_h
#define tamgufmatrix_h
...
class Tamgufmatrix;
//This typedef defines a type "fmatrixMethod", which expose the typical parameters of a new Tamgu method implementation
typedef Tamgu* (Tamgufmatrix::*fmatrixMethod)(Tamgu* contextualpattern, short idthread, TamguCall* callfunc);

//---------------------------------------------------------------------------------------------------------------------

//---------------------------------------------------------------------------------------------------------------------
class Tamgufmatrix : public TamguObject {
public:
	//We export the methods that will be exposed for our new object
	//this is a static object, which is common to everyone
	//We associate the method pointers with their names in the linkedmethods map
	static Exchanging basebin_hash<fmatrixMethod> methods;
	...
	TAMGUIMATRIX<TAMGUILINEFLOAT> values;
	...
	Tamgu* Newinstance(short, Tamgu* f = NULL) {
		return new Tamgufmatrix;
	}
	TamguIteration* Newiteration(bool direction);
	static void AddMethod(TamguGlobal* g, string name, fmatrixMethod func, unsigned long arity, string infos);
	...
	Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
		...
		locking();
		contextualpattern = (this->*Tamgufmatrix::methods.get(idname))(contextualpattern, idthread, callfunc);
        unlocking();
	...
	long LUDCMP(long n, VECTE<long>& INDX, long& d, Tamgufmatrix& m) {
	...
	void LUBKSB(long n, VECTE<long>& INDX, VECTE<double>& B, Tamgufmatrix& m)  {
		double SUM;
		long  I, II, J, LL;
	...
	double determinant(long rsize, long csize, TAMGUIMATRIX<TAMGUILINEFLOAT>& m) {
		if (rsize == 2 && csize == 2) {
			//then in that case
		...
		for (long j = 0; j < rsize; j++) {
			...
			TAMGUIMATRIX<TAMGUILINEFLOAT> sub(rsize - 1);
	...
	bool inversion(Tamgufmatrix& Y) {
		//else Local decomposition

		Y.init(rowsize, columnsize);

		Tamgufmatrix m(NULL, NULL);
		populate(&m);
	...
	Tamgufmatrix* duplicate() {
		Tamgufmatrix* ke = new Tamgufmatrix;
...
public:
	...
	TamguIterationfmatrix(Tamgufmatrix* v, bool d, TamguGlobal* g = NULL) : TamguIteration(d, g) {
		value = v;
	}
