...
class Tamgumap : public TamguObjectLockContainer {
    ...
    void Methods(Tamgu* v) {
            for (const auto& it : globalTamgu->infomethods[a_map])
                 v->storevalue(it.first);
      }

      string Info(string n) {
            if (globalTamgu->infomethods[a_map].find(n) !=  globalTamgu->infomethods[a_map].end())
              return globalTamgu->infomethods[a_map][n];
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        ...
        return (this->*Tamgumap::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
