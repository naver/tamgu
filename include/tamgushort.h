...
class Tamgushort : public TamguReference {
	...
	void Methods(Tamgu* v) {
            for (const auto& it : globalTamgu->infomethods[a_short])
                 v->storevalue(it.first);
      }

      string Info(string n) {
            if (globalTamgu->infomethods[a_short].find(n) !=  globalTamgu->infomethods[a_short].end())
              return globalTamgu->infomethods[a_short][n];
	...
	Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
		...
		return (this->*Tamgushort::methods.get(idname))(contextualpattern, idthread, callfunc);
	}
