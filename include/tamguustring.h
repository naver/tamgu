...
public:
    ...
    void Methods(Tamgu* v) {
            for (const auto& it : globalTamgu->infomethods[a_ustring])
                 v->storevalue(it.first);
      }

      string Info(string n) {
            if (globalTamgu->infomethods[a_ustring].find(n) !=  globalTamgu->infomethods[a_ustring].end())
              return globalTamgu->infomethods[a_ustring][n];
	...
	Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
		...
		return (this->*Tamguustring::methods.get(idname))(contextualpattern, idthread, callfunc);
	}
	//---------------------------------------------------------------------------------------------------------------------
	void Clear() {
		Locking _lock(this);
...
public:
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
            //This call is a bit cryptic. It takes the method (function) pointer that has been associated in our map with "name"
            //and run it with the proper parameters. This is the right call which should be invoked from within a class definition
        return (this->*Tamgua_ustring::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
        //---------------------------------------------------------------------------------------------------------------------
    void Clear() {
        value.clear();
