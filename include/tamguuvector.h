...
class Tamguuvector : public TamguLockContainer {
	...
	Exporting void storevalue(Tamgu* v, long beg, long end) {
        ...
        for (;beg < end && beg < sz; beg++)
            values.push_back(((Tamguuvector*)v)->values[beg]);
    }
    ...
    void Methods(Tamgu* v) {
            for (const auto& it : globalTamgu->infomethods[a_uvector])
                 v->storevalue(it.first);
      }

      string Info(string n) {
            if (globalTamgu->infomethods[a_uvector].find(n) !=  globalTamgu->infomethods[a_uvector].end())
              return globalTamgu->infomethods[a_uvector][n];
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
        ...
        return (this->*Tamguuvector::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
...
public:
    ...
    Tamgu* CallMethod(short idname, Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
            //This call is a bit cryptic. It takes the method (function) pointer that has been associated in our map with "name"
            //and run it with the proper parameters. This is the right call which should be invoked from within a class definition
        return (this->*Tamgua_uvector::methods.get(idname))(contextualpattern, idthread, callfunc);
    }
