 ...
 */
//  lispetokens.h (see tools.cxx for implementation)
//
//

#ifndef lispetokens_h
#define lispetokens_h
...
public:
    ...
    /*
     ...
     b) Or it is in the tokeniser_rules vector
     ...
     The tokeniser_rules contains the rule, which could not be indexed on their first characters.
     Basically when parsing a code, we first try the rules which are indexed on this character if we can,
     otherwise we roll backs on tokeniser_rules.
     */
