...
public:
    ...
    void buildList(LispE* lisp, Element* result, Element* current, vecter<long>& shape, vecter<long>& positions, long idx, long axis);
    Element* storeRank(LispE* lisp, Element* result, Element* current, vecter<long>& shape, vecter<long>& positions, long idx);
    ...
    void build(LispE* lisp, vecter<long>& shape, long isz, Element* res, Element* lst, long& idx);
    ...
    bool structuredList(long depth, vecter<long>& sz, short& element_type) {
        if (size() != sz[depth])
            return false;
    ...
    bool checkShape(long depth, vecter<long>& sz) {
        if (size() != sz[depth])
            return false;
        if (depth == sz.size()-1) {
            for (long i = 0; i < size(); i++) {
...
public:
    ...
    vecter<int16_t> names;
    int16_t space;

    List_instance(int16_t sp, int16_t lab, vecter<int16_t>& n, Element* v, long sz) : List(sz, v), names(n) {
        clean = NULL;
        type = lab;
        space = sp;
    }

    List_instance(int16_t sp, int16_t lab, vecter<int16_t>& n) : names(n) {
        clean = NULL;
        type = lab;
        space = sp;
    }
...
public:
    ...
    Tenseur<A,T,C>(long nb, vecter<long>& sz) {
        type = T;
        for (long i = 1; i < sz.size(); i++)
            shape.push_back(sz[i]);
    }
    
    Tenseur<A,T,C>(vecter<long>& sz, Element* n);
    Tenseur<A,T,C>(vecter<long>& sz, A n);
    Tenseur<A,T,C>(vecter<long>& sz, C n);
    Tenseur<A,T,C>(LispE* lisp, vecter<long>& sz, Element* n);
    
    Tenseur<A,T,C>(LispE* lisp, Element* lst, vecter<long>& sz);
    Tenseur<A,T,C>(Tenseur<A,T,C>* tensor);
    
    //We steal the ITEM structure of this list
    Tenseur<A,T,C>(LispE* lisp, List* l);
    Tenseur<A,T,C>(LispE* lisp, List* l, vecter<long>& sh, long pos);
    ...
    Element* storeRank(LispE* lisp, Element* result, Element* current, vecter<long>& positions, long idx);
    Element* rank(LispE* lisp, vecter<long>& positions);
