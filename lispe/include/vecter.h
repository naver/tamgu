...
public:
    ...
    vecter_a(vecter_a<Z>& l, long pos) {
        home  = pos + l.home;
        items = l.items;
    ...
    inline bool operator ==(vecter_a<Z>& v) {
    ...
    void minus(vecter_a<Z>& n, long nb) {
        items->minus(home, n.home, n.items->buffer, nb);
    }

    void multiply(vecter_a<Z>& n, long nb) {
    ...
    void searchall(vecter_a<long>& indexes, Z v, long ix) {
...
public:
    ...
    vecter_n(vecter_n<Z>& l, long pos) {
        home  = pos + l.home;
        items = l.items;
        //We modify the common reference counter
        items->status++;
    ...
    inline bool operator ==(vecter_n<Z>& v) {
        if (v.items != items && size() == v.size()) {
            long i = home;
            for (; i < items->last && v[i] == items->buffer[i]; i++) {}
            return (i == items->last);
