...
//------------------------------------------------------------------------------------------
Element* Element::rank(LispE* lisp, vecter<long>& positions) {
...
Element* Stringbyte::extraction(LispE* lisp, List* liste) {
    ...
    if (nxt == liste->size()) {
        //Only one element is returned
        return new Stringbyte(lisp->handlingutf8->get_a_char(content, from));
    }
...
Element* Stringbyte::car(LispE* lisp) {
    if (content.size() == 0)
        throw new Error("Error: Empty string");
    long i = 0;
    return new Stringbyte(lisp->handlingutf8->get_a_char(content, i));
}
...
Element* Stringbyte::cadr(LispE* lisp, u_ustring& action) {
    ...
    for (i = action.size() - 1; i>= 0; i--) {
        if (action[i] == 'a') {
            if (i)
                throw new Error("Error: cannot apply 'car/cdr' to one character");
            u = lisp->handlingutf8->get_a_char(content, pos);
