...
void Delegation::initialisation(LispE* lisp) {
    ...
    set_instruction(l_defprol, "defprol", LP_ATLEASTFOUR, &List::evall_defprol);
    set_instruction(l_defspace, "defspace", LP_TWO, &List::evall_defspace);
    set_instruction(l_class, "class@", LP_ATLEASTFOUR, &List::evall_class);
    set_instruction(l_from, "from@", LP_ATLEASTTWO, new List_from());
    ...
    set_instruction(l_toclean, "toclean", LP_TWO,  &List::evall_toclean);
...
#ifdef LISPE_WASM 
    set_instruction(l_evaljs, "evaljs", LP_TWO, &List::evall_js);
    set_instruction(l_evaljssync, "asyncjs", LP_ATLEASTTWO, &List::evall_js_sync);
#else
    ...
    set_instruction(l_setfast, "setfast", LP_THREE, new List_setfast_eval());
    set_instruction(l_getfast, "getfast", LP_TWO, new List_getfast_eval());
    ...
    set_instruction(l_setqv, "setqv", LP_THREE, &List::evall_setqv, new List_setqv_eval());
    set_instruction(l_setqi, "setqi", LP_THREE, &List::evall_setqi, new List_setqi_eval());
    ...
    set_instruction(l_withclass, "withclass", LP_ATLEASTTWO, new List_withclass_eval());
    ...
    set_instruction(l_this, "this", LP_ONE, &List::evall_this);
    ...
    set_instruction(l_rank, "rank@", LP_ATLEASTTWO,  new List_rank_eval());
    set_instruction(l_equalonezero, "==", LP_THREE, &List::evall_equalonezero, new List_equalonezero_eval());
    set_instruction(l_rho, "ρ", LP_ATLEASTTWO,  new List_rho_eval());
...
Element* Delegation::tokenize(LispE* lisp, u_ustring& code, bool keepblanks, short decimalseparator, bool locking) {
    ...
    tokeniser_result<u_ustring> tokres(&res->liste, false);
...
Element* Delegation::tokenize(string& code, bool keepblanks, short decimalseparator, bool locking) {
    ...
    tokeniser_result<string> tokres(&res->liste, false);
        ...
        switch(lab) {
            ...
            case l_format: {
                if (element->size() == 2) {
                    ...
                    u = str_ureplacestring(u, U"%{", op_rep);
                    u = str_ureplacestring(u, U"%}", cl_rep);
                    long posinit = 0;
                    ...
                    for (u_ustring s : v) {
                        if (s[0] == '{' && s.back() == '}') {
                            ...
                            s = str_ureplacestring(s, op_rep, U"{");
                            s = str_ureplacestring(s, cl_rep, U"}");
