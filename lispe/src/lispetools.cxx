...
#ifdef WIN32
 void concat_to_wstring(wstring& res, UWCHAR code) {
...
long utf8_handler::c_bytetoutf16position(string& contenu, long charpos) {
    ...
    UWCHAR code;
    while (i < charpos) {
        i += 1 + chr_utf8_to_unicode(contenu, i, code);
        if (code & 0xFFFF0000)
            sz++;
        sz++;
    }
...
UWCHAR getonechar(unsigned char* s, long& i) {
    UWCHAR result, code;
    i += chr_utf8_to_unicode(s + i, code);
    if (chr_utf16_to_unicode(result, code, false)) {
        i += chr_utf8_to_unicode(s + i, code);
        chr_utf16_to_unicode(result, code,  true);
    }
    return result;
}
...
unsigned char c_unicode_to_utf8(UWCHAR code, unsigned char* utf) {
 ...
 char* u_unicode_2_utf8(long code, char* utf) {
    unsigned char c = (bool(code & 0xFFFF0000) << 3) | (bool(code & 0xF800)<<2) | (bool(code & 0x780) << 1);
    switch (c) {
        case 8:
        case 10:
...
string c_unicode_to_utf8(UWCHAR code) {
 ...
 char chr_test_utf8(string& utf, long i) {
    if (i == utf.size())
        return 0;
 ...
 char chr_test_utf8(unsigned char* utf) {
    if (utf == NULL)
        return 0;
...
utf8_handler::utf8_handler() {
 ...
 unsigned char chr_utf8_to_unicode(unsigned char* utf, UWCHAR& code) {
    code = utf[0];
 ...
 unsigned char chr_utf8_to_unicode(string& utf, long i, UWCHAR& code) {
    code = utf[i];
...
//--------------------------------------------------------------------
 ...
 wstring wjsonstring(u_ustring value) {
    return wjsonstring(_u_to_w(value));
}

 u_ustring ujsonstring(u_ustring value) {
    if (value == U"")
        return U"\"\"";
    ...
    if (value.find(U"\\") != -1)
        value = str_ureplacestring(value, U"\\", U"\\\\");
...
bool utf8_handler::s_is_space(string& str) {
	...
	UWCHAR code;
    for (long i = 0; i < lg; i++) {
        i += chr_utf8_to_unicode(str, i, code);
        if (!c_is_space(code))
            return false;
    }
    return true;
...
bool s_is_digit_l(string& str) {
 ...
 bool s_is_utf8(string& contenu, long longueur) {
    long i = 0;
    while (longueur--) {
        if ((contenu[i] & 0x80) && chr_test_utf8(contenu, i))
            return true;
        i++;
    }
    return false;
...
long getindex_l(string& contenu, long lg, long i) {
    long x = 0;
 ...
 long size_raw_c(string& contenu, long sz) {
    long i = 0;
    long size = i;
    long nb;
    
    while (i < sz) {
        nb = chr_test_utf8(contenu, i);
        i += nb + 1;
        ++size;
    }
...
string s_middle_l(string& s, long l, long nb) {
    long lg = s.size();

    long i = getindex_l(s, lg, l);
    if (i >= lg)
        return "";
...
void no_conversiontofloathexa(const char* s, int sign, int16_t& l) {
    uchar c = *s++;
    l++;
    ...
    if (isadigit(*s)) {
        if (*s=='0') {
            if (s[1]=='x') {
                s+=2;
                l++;
                no_conversiontofloathexa(s, sign, l);
                return;
            }
...
void no_conversiontofloathexa(wchar_t* s, int sign, long& l) {
    wchar_t c = *s++;
    l++;
    while (c < 103 && digitaction[c]) {
        c = *s++;
...
//------------------------------------------------------------------------
 ...
 void str_utf16_to_utf8(string& s, int32_t* str, long sz) {
    s = "";
    if (!sz)
        return;
 ...
 void str_utf16_to_utf8(string& s, wchar_t* str, long sz) {
	if (!sz)
		return;
 ...
 void str_utf16_to_utf8(string& s, u_ustring& str) {
    long sz = str.size();
    if (!sz)
        return;
 ...
 void str_unicode_to_utf8(string& s, wstring& str) {
	str_utf16_to_utf8(s, str);
}

 void str_unicode_to_utf8(string& s, wchar_t* str, long sz) {
...
void str_unicode_to_utf8(string& s, u_ustring& str) {
 ...
 void s_utf8_to_utf16(wstring& w, string& str , long sz) {
    if (!sz)
        return;
    ...
    while (sz--) {
        if (str[i] & 0x80) {
            nb = chr_utf8_to_unicode(str, i, c);
            i += nb + 1;
            sz = (sz >= nb)?sz-nb:0;
            if (!(c & 0xFFFF0000)) {
                w += (wchar_t)c;
 ...
 void s_utf8_to_unicode_u(wstring& w, unsigned char* str , long sz) {
    if (!sz)
        return;
 ...
 void s_utf8_to_unicode(wstring& w, string& str , long sz) {
    if (!sz)
        return;
 ...
 void s_utf8_to_unicode(u_ustring& w, string& str , long sz) {
    if (!sz)
        return;
...
//------------------------------------------------------------------------
void str_split(string& s, string& splitter, vector<string>& vs, bool keepblanks) {
    size_t pos = 0;
    size_t found = 0;
    string sub;
...
void str_split(wstring& s, wstring& splitter, vector<wstring>& vs, bool keepblanks) {
    size_t pos = 0;
    size_t found = 0;
    wstring sub;
    long sz = splitter.size();
...
long IndentationCode_l(string& codestr) {
...
long VirtualIndentation(string& codestr, bool lisp, bool python) {
    ...
    return IndentationCode_l(codestr);
}


void IndentationCode_l(string& str, string& codeindente) {
    string token;
...
void IndentCode_l(string& codestr, string& codeindente, long blancs, bool lisp, bool python) {
    ...
    if (blancs)
        blanksize = blancs;
    codeindente = "";
    IndentationCode_l(codestr, codeindente);
    blanksize = bl;
    if (codeindente.find("/@") != string::npos || codeindente.find("@\"") != string::npos)
        cr_normalise_l(codeindente);
...
//Convert a unicode character into a utf16 character
 bool c_unicode_to_utf16(u_uchar& res, u_uchar code) {
    //A unicode character is encoded over 4 bytes: 3 -> 0
    //if we have bits on byte 2, then we need to provide 4 bytes...
    if ((code & 0x1F0000) == 0) {
        res = code;
 ...
 bool chr_utf16_to_unicode(u_uchar& r, u_uchar code, bool second) {
    if (second) {
        r |= code & 0x3FF;
        return false;
    }
...
void s_utf16_to_unicode(u_ustring& u, int32_t* w, long sz) {
    ...
    u_uchar c;
    for (long i = 0; i < sz; i++) {
        if (chr_utf16_to_unicode(c, (u_uchar)w[i], false))
            chr_utf16_to_unicode(c, (u_uchar)w[++i], true);
        u += c;
    }
}
...
void tokenizer_theautomaton::display(tokeniser_node* a, int nbblanks, bool top) {
    ...
    if (a->action == actl_meta) {
        cout << '%';
    }
    
    if (a->action == actl_end) {
        if (a->label < 33)
            cout << '$' << (int)a->label;
        else
            cout << '$' << (uchar)a->label;
...
bool tokeniser_node::check(u_uchar chr, utf8_handler* access, std::set<u_uchar>& operators, bool not_neg) {
    ...
    switch(action) {
        ...
        case actl_ckj://Chinese/Japanese/Korean
            return (not_neg == ckjchar(chr));
        case actl_space_and_carriage:
            return (not_neg == c_is_space_or_cr(chr));
        case actl_alphabetical:
        ...
        case actl_non_breaking_space: //non-breaking space
            return (not_neg == c_is_nbs_space(chr));
        case actl_operator:
            return (not_neg == operators.count(chr));
        case actl_punctuation:
...
void tokeniser_node::processing_postponed_nodes(std::set<long>& visited, vector<tokeniser_node*>& nodes, long idpos) {
    if (check_seen())
        return;
    
    add_flag(flags, flagl_seen);
...
void tokeniser_node::remove_epsilon_nodes(std::unordered_map<long,
                                          std::set<long> >& visited, std::set<long>& current,
                                          vector<tokeniser_node*>& nodes, bool epsilon) {
    if (check_visited())
        return;
    ...
    if (sz) {
        ...
        long key;
        std::unordered_map<long, tokeniser_node*> commons;
        vector<tokeniser_node*> nds;
        for (i = 0; i < sz; i++) {
            n = arcs[i];
            if (n->pure_arc()) {
                key = n->action*256 + n->label;
                if (commons.count(key)) {
                    tokeniser_node* nbase = commons[key];
    ...
    for (i = 0; i < sz; i++) {
        n = arcs[i];
        if (n->is_epsilon()) {
            if (n->size() == 1 && n->arcs[0]->action == actl_end) {
                add_flag(n->arcs[0]->flags, flagl_visited);
                n->clone(n->arcs[0]);
                n->arcs.clear();
                vect->insert(n->idpos);
            }
                ...
                if (n->check_visited()) {
                    if (visited.count(n->idpos)) {
                        ...
                        add_flag(n->flags, flagl_postpone);
                        vect->insert(n->idpos);
                    }
                }
                else {
...
//----------------------------------------------------------------------------------------
//Specialization of extracting one character from a buffer for tokeniser_result
//tokeniser_result is a template that depends on the type of the string being manupulated.
//However each type has a specific expected behaviour
//----------------------------------------------------------------------------------------

template<> void tokeniser_result<u_ustring>::getnext() {
...
template<> void tokeniser_result<wstring>::getnext() {
    if (position >= buffer_size) {
        currentchr =  0;
        return;
    }
    ...
    sz_read = 1;
    line += (currentchr == '\n');
    if (chr_utf16_to_unicode(currentchr, (*buffer)[position++], false)) {
        sz_read = 2;
        chr_utf16_to_unicode(currentchr, (*buffer)[position++], true);
    }
}

template<> void tokeniser_result<u16string>::getnext() {
    if (position >= buffer_size) {
        currentchr =  0;
        return;
    }
...
template<> void tokeniser_result<string>::getnext() {
    if (position >= buffer_size) {
        currentchr =  0;
        return;
    }
    ...
    line += (currentchr == '\n');
    sz_read = 1 + chr_utf8_to_unicode(buffer, position, currentchr);
    position += sz_read;
}



template<> void tokeniser_result<u_ustring>::store_currentchr() {
...
#ifdef WIN32
template<> void tokeniser_result<wstring>::store_currentchr() {
    u_uchar c;
    wstring w;
...
template<> void tokeniser_result<std::u16string>::store_currentchr() {
    u_uchar c;
    u16string w;
    if (c_unicode_to_utf16(c, currentchr)) {
        w = (char16_t)(c >> 16);
...
template<> void tokeniser_result<string>::store_currentchr() {
...
template<> void tokeniser_result<u_ustring>::store(int32_t label) {
    if (label != -1) {
...
#ifdef WIN32
template<> void tokeniser_result<wstring>::store(int32_t label) {
    if (label != -1) {
        wstring w;
        str_unicode_to_utf16(w, token);
        stack_ptr->push_back(w);
        if (store_all) {
            stacktype.push_back((unsigned char)label);
            stackln.push_back(line);
...
template<> void tokeniser_result<u16string>::store(int32_t label) {
    if (label != -1) {
        u16string w;
        str_unicode_to_utf16(w, token);
        stack_ptr->push_back(w);
...
template<> void tokeniser_result<string>::store(int32_t label) {
    if (label != -1) {
        string w;
        str_unicode_to_utf8(w, token);
        stack_ptr->push_back(w);
        if (store_all) {
            stacktype.push_back((unsigned char)label);
            stackln.push_back(line);
...
void tokenizer_theautomaton::compile() {
    /*
     ...
     This table contains: 1611 elements (see character_table_size).
     ...
     actl_char             -> regular character
     actl_meta             -> meta-character (%x)
     actl_interval         -> interval between two characters: #A-F
     actl_any              -> any character
     actl_epsilon          -> an epsilon arc, which has created to handle disjunction or optionalaties
     actl_end              -> the termination act, associated with a rule identifier, the returned value when the rule has applied
     
     We also have different flags to handle the node status
     flagl_negation (1)        -> negation (~)
     flagl_skip (2)            -> Character not stored (-)
     flagl_fail (4)            -> Set to Element in negated disjunctions. When element is true, rule fails thank to this flag.
     flagl_action (7)          -> flagl_negation | flagl_skip | flagl_fail
     
     //The next flags are used to mark node utilisation
     flagl_vectorized (8)   //has been added to vectorization_table as an index based on the first value of the rule
     flagl_added (16)       //has been added to vectorization_table at position 0
     flagl_postpone (32)    //While removing epsilon, a node that has already been seen but not fully evaluated since
     flagl_visited (64)     //A node that has already been visited
     flagl_seen (128)       //A node that has already been seen when processing postponed nodes
    ...
    vector<tokeniser_node*> brackets;
    vector<char> currentbracket;
    
    u_ustring line;
    u_ustring action_id;
    
    tokeniser_node* anode = NULL;
    tokeniser_node* current = NULL;
    tokeniser_node* root = NULL;
    ...
    for (irule = 0; irule < rules.size() && !error; irule++) {
            ...
            switch(cc) {
                    ...
                    if (first) {
                        ...
                        switch (cc) {
                            case 'C': //Upper-case characters
                                anode = node(actl_meta, cc);
                                for (e = 'A'; e < character_table_size; e++) {
                ...
                case '+':
                    if (pos) { //only if it is not the first character
                        ...
                        if (check_flag(current->action, actl_epsilon)) {
                            anode = node(actl_epsilon, 0);
                            for (e = 0; e < current->size(); e++) {
                ...
                case '#': //introduce an interval or a code
                    //interval
                    if (line[pos+2] == '-') {
                        anode = node(actl_interval, line[pos+1], line[pos+3]);
                        if (first) {
                ...
                case '-':
                    if (pos)
                        add_flag(current->flags, flagl_skip);
                    else
                        error = 11;
                    break;
                case '~':
                    negation = flagl_negation;
                    break;
                case '?':
                    anode = node(actl_any, cc);
                    root = current;
                    current = append(current, anode);
                    break;
        ...
        if (final_action == -2) {
            ...
            add_flag(initial->flags, flagl_blocking_gate);
        }
        
        anode = node(actl_end, final_action);
        append(current, anode);
        
        if (displayautomata) {
            string s;
            str_unicode_to_utf8(s, line);
            cout << s << endl;
        ...
        if (tail) {
            ...
            while (anode->arcs.size() == 1)
                anode = anode->arcs[0];
            add_flag(anode->flags, flagl_tail);
        }
    ...
    //Removing all marks except flagl_negation, flagl_skip and flagl_fail
    for (auto& nd : nodes)
        nd->flags &= flagl_action;
    ...
    for (e = 1; e < character_table_size; e++) {
    ...
    vector<tokeniser_node*> intermediary;
    for (auto& nd : nodes) {
        if (!nd->check_visited())
            delete nd;
        else {
...
void tokenizer_theautomaton::setrules() {
    /*
     ...
     ...=!NN -> '=!' means that the rule has been flagged with flagl_blocking_gate. It is basically deactivated.
     ...=# -> the '#' means that the token will be recognized but not stored in the final list of tokens
     
     The flagl_blocking_gate can be used to activate or deactivate rules on the fly
     */
