...
void List::buildList(LispE* lisp, Element* result, Element* current, vecter<long>& shape, vecter<long>& positions, long idx, long axis) {
    if (axis < idx) {
...
Element* List::storeRank(LispE* lisp, Element* result, Element* current, vecter<long>& shape, vecter<long>& positions, long idx) {
    long axis = idx;
...
Element* List::rank(LispE* lisp, vecter<long>& positions) {
    vecter<long> shape;
    getShape(shape);
...
//--------------------------------------------------------------------------------
/*
template<> Element* vecter_a<long>::provide(LispE* lisp, long v) {
    return lisp->provideInteger(v);
}
*/

template <class Z> bool vecter_a<Z>::compare(LispE* lisp, List* comparison, int16_t instruction, long i, long j) {
    comparison->liste[1]->setvalue(items->buffer[i]);
    comparison->liste[2]->setvalue(items->buffer[j]);
    return comparison->eval_Boolean(lisp, instruction);
}

template <class Z>void vecter_a<Z>::values_sorting(LispE* lisp, List* comparison, int16_t instruction, long rmin, long rmax) {
    //(setq s (sort '< (shuffle (cons 5 (range 1 99999 1)))))
    //(sort '< '(28 60 10 38 80 34 8 22 78 68 85 48 13 39 100 56 89 82 11 52 99 50 20 96 97 59 23 81 53 15 3 67 77 7 57 74 49 32 86 66 43 26 75 62 29 71 2 91 51 1 18 12 24 21 36 72 90 40 70 14 61 93 6 4 79 94 47 58 30 83 84 44 88 63 95 45 33 65 37 92 27 64 55 9 31 73 54 16 98 5 46 25 76 42 17 69 19 35 5 41 87))
    //(sort '< '(20 12 15 13 19 17 14))
    //(sort '< (shuffle (range 1 16 1)))
...
template <class Z> bool vecter_n<Z>::compare(LispE* lisp, List* comparison, int16_t instruction, long i, long j) {
    comparison->liste[1]->setvalue(items->buffer[i]);
    comparison->liste[2]->setvalue(items->buffer[j]);
    return comparison->eval_Boolean(lisp, instruction);
}

template <class Z> void vecter_n<Z>::values_sorting(LispE* lisp, List* comparison, int16_t instruction, long rmin, long rmax) {
    //(setq s (sort '< (shuffle (cons 5 (range 1 99999 1)))))
    //(sort '< '(28 60 10 38 80 34 8 22 78 68 85 48 13 39 100 56 89 82 11 52 99 50 20 96 97 59 23 81 53 15 3 67 77 7 57 74 49 32 86 66 43 26 75 62 29 71 2 91 51 1 18 12 24 21 36 72 90 40 70 14 61 93 6 4 79 94 47 58 30 83 84 44 88 63 95 45 33 65 37 92 27 64 55 9 31 73 54 16 98 5 46 25 76 42 17 69 19 35 5 41 87))
    //(sort '< '(20 12 15 13 19 17 14))
    //(sort '< (shuffle (range 1 16 1)))
