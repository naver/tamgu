...
public:
    ...
    virtual void rulestring(std::wstringstream& str, tokeniser_node* a) {}
    
    virtual u_ustring rule(long pos) {
        return U"";
    }

    virtual tokeniser_node* compiled_rule(long pos) {
        return NULL;
    }
    ...
    virtual void setrules(vecter_n<u_ustring>& v) {}
...
public:
    ...
    void rulestring(std::wstringstream& str, tokeniser_node* a) {
    ...
    Element* tokenize(u_ustring& u, bool types) {
        ...
        tokeniser_result<u_ustring> tokres(&res->liste, types);
    ...
    Element* tokenize(string& u, bool types) {
        ...
        tokeniser_result<string> tokres(&res->liste, types);
        tok->tokenize(u, tokres);
        if (!types)
            return res;
        List* l = lisp->provideList();
    ...
    void setrules(vecter_n<u_ustring>& v) {
...
public:
    ...
    Element* method_ngrams(LispE* lisp, long nb, Element* vstr) {
        ...
        for (long i = 0; i < mx; i++) {
            u = "";
            for (j = i; j < i + nb; j++) {
                u += lisp->handlingutf8->get_a_char(s,j);
            }
