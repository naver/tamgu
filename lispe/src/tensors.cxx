...
//------------------------------------------------------------------------------------------
Element* LispE::tensor_to_lispe(Element* e, vecter<long>& shape) {
    Element* res = n_null;
...
//LU decomposition
long LUDCMP(long n, vecter<long>& indexes, long& d, Matrice_number* m) {
    d = 1;
...
void LUBKSB(long n, vecter<long>& indexes, vecter<double>& b_values, Matrice_number* m)  {
    double thesum;
    long  i, ii, j, ll;
...
long LUDCMP(long n, vecter<long>& indexes, long& d, Matrice_float* m) {
    d = 1;
...
void LUBKSB(long n, vecter<long>& indexes, vecter<float>& b_values, Matrice_float* m)  {
    float thesum;
    long  i, ii, j, ll;
...
long LUDCMP(long n, vecter<long>& indexes, long& d, Matrice_integer* m) {
    d = 1;
    float AMAX, DUM, thesum;
    long i, i_max = 0, j, k;
    vector<long> values;
...
void LUBKSB(long n, vecter<long>& indexes, vecter<long>& b_values, Matrice_integer* m)  {
    long thesum;
    long  i, ii, j, ll;
...
template <typename A, lisp_code T, lisp_code TT, typename C> Element* Matrice<A,T,TT,C>::rank(LispE* lisp, vecter<long>& positions) {
...
template <typename A, lisp_code T, typename C> Tenseur<A,T,C>::Tenseur(vecter<long>& sz, Element* n) {
    type = T;
    shape = sz;
    if (shape.size())
...
template <typename A, lisp_code T, typename C> Tenseur<A,T,C>::Tenseur(vecter<long>& sz, A n) {
    type = T;
    shape = sz;
    if (shape.size())
        build(0,this, n);
...
template <typename A, lisp_code T, typename C> Tenseur<A,T,C>::Tenseur(vecter<long>& sz, C n) {
    type = T;
    shape = sz;
    long idx = 0;
    if (shape.size())
...
template <typename A, lisp_code T, typename C> Tenseur<A,T,C>::Tenseur(LispE* lisp, vecter<long>& sz, Element* n) {
    type = T;
    shape = sz;
    if (shape.size())
...
template <typename A, lisp_code T, typename C> Tenseur<A,T,C>::Tenseur(LispE* lisp, Element* lst, vecter<long>& sz) {
    type = T;
    shape = sz;
    if (shape.size()) {
        long idx = 0;
...
template <typename A, lisp_code T, typename C> Tenseur<A,T,C>::Tenseur(LispE* lisp, List* l, vecter<long>& sh, long pos) : List(l,pos) {
    type = T;
    shape = sh;
    shape.vecteur[0] -= pos;
}
...
template <typename A, lisp_code T, typename C> Element* Tenseur<A,T,C>::storeRank(LispE* lisp, Element* result, Element* current, vecter<long>& positions, long idx) {
    long axis = idx;
...
template<> Element* Tenseur_short::rank(LispE* lisp, vecter<long>& positions) {
...
template<> Element* Tenseur_integer::rank(LispE* lisp, vecter<long>& positions) {
    //We get rid of the final negative values (useless)
    int16_t sz = positions.size();
    if (!sz || sz > shape.size())
        sent_error("Error: index mismatch");
...
template<> Element* Tenseur_string::rank(LispE* lisp, vecter<long>& positions) {
    //We get rid of the final negative values (useless)
    int16_t sz = positions.size();
    if (!sz || sz > shape.size())
        sent_error("Error: index mismatch");
...
template<> Element* Tenseur_stringbyte::rank(LispE* lisp, vecter<long>& positions) {
    //We get rid of the final negative values (useless)
    int16_t sz = positions.size();
    if (!sz || sz > shape.size())
        sent_error("Error: index mismatch");
...
template<> Element* Tenseur_float::rank(LispE* lisp, vecter<long>& positions) {
    //We get rid of the final negative values (useless)
    int16_t sz = positions.size();
    if (!sz || sz > shape.size())
        sent_error("Error: index mismatch");
...
template<> Element* Tenseur_number::rank(LispE* lisp, vecter<long>& positions) {
    //We get rid of the final negative values (useless)
    int16_t sz = positions.size();
    if (!sz || sz > shape.size())
        sent_error("Error: index mismatch");
