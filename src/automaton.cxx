...
/* --- CONTENTS ---
 ...
 Date       : 2017/09/01
 Purpose    : Double-Sided automaton implementation to handle lexicons
 Programmer : Claude ROUX (claude.roux@naverlabs.com)
 Reviewer   :
*/

//----------------------------------------------------------------------------------
//------------ TamguDoubleSideAutomaton Section -----------------------------------------------
//----------------------------------------------------------------------------------
#include "tamgu.h"
#include "automaton.h"
#include "longmap.h"
...
void TamguDoubleSideAutomaton::merge(TamguDoubleSideAutomaton* a) {
    if (a==NULL)
        return;
...
bool TamguDoubleSideAutomaton::process(charRead& w, vector<string>& readings, bool option, long threshold, short flags, short idthread) {
	if (!finalize) {
		fillencoding(true);
		start.shuffle();
	}
...
FstCompanion* ThreadStruct::Companion() {
    if (fstcompanion==NULL)
        fstcompanion =  new FstCompanion;

    return fstcompanion;
}
    
bool TamguDoubleSideAutomaton::up(wstring& w, vector<string>& res, long threshold, short flags, short idthread) {
    if (!finalize)
        fillencoding(true);
...
bool TamguFst::up(FstCompanion* f, long pos, long threshold, short flags) {
	...
	TAMGUCHAR cr = f->w[pos];
	TAMGUCHAR cw = f->a->code(cr);
...
#ifdef WSTRING_IS_UTF16
bool TamguDoubleSideAutomaton::down(wstring& str, vector<string>& res, short idthread, char lemma) {
	if (!finalize)
		fillencoding(true);

	unicodestring w(str);
#else
bool TamguDoubleSideAutomaton::down(wstring& w, vector<string>& res, short idthread, char lemma) {
	if (!finalize)
		fillencoding(true);
...
#endif
			w[fpos] = (TAMGUCHAR)multis[sortedmultis[i]];
		}
	}
...
void TamguDoubleSideAutomaton::fillencoding(bool add) {
	finalize = true;
