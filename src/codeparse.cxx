...
char bnf_tamgu::m_sousbloc_0_1_2(string& lreturn,x_node** tree) {
    ...
    //BODYOR
    subtree=NULL;
    if (m_hdata(lret,&subtree) || m_hontology(lret,&subtree) || m_telque(lret,&subtree))
        x_init_tree(tree,subtree,addsubtree);
    else {
        x_pop_node(tree,addsubtree);
        currentpos=pos;
...
char bnf_tamgu::m_lispexpression(string& lreturn,x_node** tree) {
    if (gFail==1) return(0);
...
char bnf_tamgu::m_lispexpressions(string& lreturn,x_node** tree) {
    if (gFail==1) return(0);
...
char bnf_tamgu::m_onelisp_0_1(string& lreturn,x_node** tree) {
    if (gFail==1) return(0);
    string lret;
    long pos=currentpos;
    int itok=intoken;
...
char bnf_tamgu::m_onelisp(string& lreturn,x_node** tree) {
    if (gFail==1) return(0);
...
char bnf_tamgu::m_lisp(string& lreturn,x_node** tree) {
    if (gFail==1) return(0);
