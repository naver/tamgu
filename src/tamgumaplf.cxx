...
void Tamgumaplf::AddMethod(TamguGlobal* global, string name, maplfMethod func, unsigned long arity, string infos) {
  ...
  if (Tamgumaplf::methods.isEmpty())
   ...
   bool Tamgumaplf::InitialisationModule(TamguGlobal* global, string version) {
    ...
    if (version != "") {
        global->returnindextypes[Tamgumaplf::idtype] = a_float;        
...
Tamgu* Tamgumaplf::EvalWithSimpleIndex(Tamgu* key, short idthread, bool sign) {
...
Exporting Tamgu* Tamgumaplf::divideinteger(Tamgu* b, bool itself) {
    Doublelocking _lock(this, b);
