...
void Tamgurawstring::Setidtype(TamguGlobal* global) {
    ...
    if (Tamgurawstring::methods.isEmpty())
...
Tamgu* Tamgurawstring::Loopin(TamguInstruction* ins, Tamgu* context, short idthread) {
    ...
    Tamgu* a = aNULL;
    bool testcond = false;
    for (long i = 0; i < stringsize && !testcond; i++) {
        a->Releasenonconst();
        v.value =  value[i];
...
Tamgu* Tamgurawstring::MethodStartWith(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
...
Tamgu* Tamgurawstring::MethodOrd(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
    ...
    if (s.size() >= 1) {
        ...
        return globalTamgu->ProvideConstint((long)s[0]);
    }
...
Tamgu* Tamgurawstring::MethodParenthetic(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
    string str = String();
    string copen("(");
    string cclose(")");
...
Tamgu* Tamgurawstring::MethodByteposition(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
    agnostring s(String());
    return globalTamgu->ProvideConstint(s.chartobyteposition(callfunc->Evaluate(0, contextualpattern, idthread)->Integer()));
}

Tamgu* Tamgurawstring::MethodCharposition(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
    agnostring s(String());
    return globalTamgu->ProvideConstint(s.bytetocharposition(callfunc->Evaluate(0, contextualpattern, idthread)->Integer()));
}
...
Tamgu* Tamgurawstring::MethodSplit(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
    ...
    if (tamgusplitter->isRegular()) {
        ...
        if (tamgusplitter->Type() == a_preg) {
            string str = String();
            tamgusplitter->splitting(kvect, str);
            return kvect;
        }
...
#ifdef INTELINTRINSICS
    find_intel_all(value, sz, thesplitter, *values);
#else
    string str = String();
    s_findall(str, thesplitter, *values);
#endif
