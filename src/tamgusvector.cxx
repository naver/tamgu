...
void Tamgusvector::AddMethod(TamguGlobal* global, string name, svectorMethod func, unsigned long arity, string infos) {
  ...
  if (Tamgusvector::methods.isEmpty())
...
bool Tamgusvector::InitialisationModule(TamguGlobal* global, string version) {
    ...
    if (version != "") {
        global->returnindextypes[a_svector] = a_string;
...
Exporting Tamgu* Tamgusvector::getvalue(long i) {
    locking();
    if (i < 0 || i >= values.size()) {
        unlocking();
        return aNOELEMENT;
    }
    Tamgu* r = globalTamgu->Providestring(values[i]);
    unlocking();
    return r;
}
...
Exporting void Tamgusvector::Setstring(string& res, short idthread) {
    res = "[";
    bool beg = true;
    string sx;
    locking();
...
Exporting Tamgu* Tamgua_svector::getvalue(long i) {
    if (i < 0 || i >= values.size())
        return aNOELEMENT;
...
Exporting void Tamgua_svector::Setstring(string& res, short idthread) {
    res = "[";
    bool beg = true;
    string sx;
    atomic_value_vector_iterator<atomic_string> it(values);
