...
Exporting Tamgu* Tamguuvector::getvalue(BLONG i) {
    ...
    if (i < 0 || i >= values.size())
        return aNOELEMENT;
    return globalTamgu->Provideustring(values[i]);
}
...
Exporting Tamgu* Tamguuvector::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
    Locking _lock(this);
    ...
    if (keyright == NULL)
        return globalTamgu->Provideustring(values[ikey]);
...
Exporting Tamgu* Tamgua_uvector::Get(Tamgu* contextualpattern, Tamgu* idx, short idthread) {
